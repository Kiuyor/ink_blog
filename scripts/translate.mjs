...
function pickBackend() {
  if (cfg.deeplKey) return "deepl";
  if (cfg.openaiKey) return "openai";
  throw new Error("未配置翻译后端：请设置 DEEPL_API_KEY 或 OPENAI_API_KEY");
}
...
async function translateDeeplBody(body) {
  ...
  const translated = await translateDeepl(m2);
...
async function translateDeeplFrontmatter(data) {
  const out = { ...data };
  if (out.title) out.title = await translateDeepl(out.title);
  if (out.description) out.description = await translateDeepl(out.description);
  if (cfg.translateTags) {
    if (Array.isArray(out.tags)) out.tags = await Promise.all(out.tags.map((t) => translateDeepl(t)));
    if (out.category) out.category = await translateDeepl(out.category);
  }
  return out;
}
